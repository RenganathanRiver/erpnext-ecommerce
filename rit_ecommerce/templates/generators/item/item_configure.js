class ItemConfigure {
	...
	get_html_for_item_found({ filtered_items_count, filtered_items, exact_match, product_info }) {
		...
		const item_view_variant = one_item ? `
			<br>
    		<br>
    		<button data-item-code="${one_item}"
        		class="btn btn-primary btn-view-variant w-50"
        		data-action="btn_view_variant"
    		>
        		<span class="mr-2">
	...
	btn_view_variant(e) {
		...
		frappe.call({
			method: 'rit_ecommerce.custom.get_variant_url',
			args: { item_code },
			callback: (response) => {
				const variantUrl = response.message;
	
				if (variantUrl) {
					this.dialog.hide();
					window.location.href = variantUrl;
				} else {
					frappe.msgprint("Website item not published");
					this.dialog.hide();
				}
			},
		});
